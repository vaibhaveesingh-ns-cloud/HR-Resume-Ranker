        ...
        <div className="flex-grow">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-slate-400" title={candidate.file_name}>{candidate.file_name}</p>
            </div>
            <a
          ...
          <div className="mt-4 pt-4 border-t border-slate-200">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              <div className={`flex items-center gap-2 text-xs ${
                candidate.python_proficiency ? 'text-green-600' : 'text-red-600'
              }`}>
                <span className={`w-2 h-2 rounded-full ${
                  candidate.python_proficiency ? 'bg-green-500' : 'bg-red-500'
                }`}></span>
                Python Skills
              </div>
              <div className={`flex items-center gap-2 text-xs ${
                candidate.ai_library_experience ? 'text-green-600' : 'text-red-600'
              }`}>
                <span className={`w-2 h-2 rounded-full ${
                  candidate.ai_library_experience ? 'bg-green-500' : 'bg-red-500'
                }`}></span>
                AI Libraries
              </div>
              <div className={`flex items-center gap-2 text-xs ${
                candidate.ml_exposure ? 'text-green-600' : 'text-red-600'
              }`}>
                <span className={`w-2 h-2 rounded-full ${
                  candidate.ml_exposure ? 'bg-green-500' : 'bg-red-500'
                }`}></span>
                ML/LLM Exposure
              </div>
              <div className={`flex items-center gap-2 text-xs ${
                candidate.ai_project_evidence ? 'text-green-600' : 'text-red-600'
              }`}>
                <span className={`w-2 h-2 rounded-full ${
                  candidate.ai_project_evidence ? 'bg-green-500' : 'bg-red-500'
                }`}></span>
                AI Projects
              </div>
            </div>
